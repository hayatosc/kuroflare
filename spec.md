...
現在の `packages/core/src/sync/meta.ts` 実装は、Worker と plugin が共有する最小 schema guard を持つ。
...
- `packages/obsidian-plugin/src/sync/obsidian/lifecycle.ts` は `runResumeTick(reason)` と `createSyncRuntimeObsidianResumePort(input)` を持ち、resume gate、startup tick、foreground WebSocket/state-vector exchange、outbox tick scheduling を production lifecycle adapter 経由で順序実行する。`main.ts` は layout ready、window focus、visibility resume、online event で adapter の resume tick を呼ぶだけに寄せ、document visibility / auth block gate も resume port factory 経由にした。
- `packages/obsidian-plugin/src/sync/engine/worker.ts` は fake vault / production adapter から使える `runOutboxWorkerLocalSideEffect()` を持ち、blob PUT、blob GET、manifest PUT、materialize の I/O を明示 port 経由で実行する。`packages/obsidian-plugin/src/sync/engine/worker.test.ts` は fake harness で scheduler tick、lease acquire、blob upload、manifest write、blob download、cache write、materialize、success completion、WebSocket Ack completion を一連で進めることを固定している。`packages/obsidian-plugin/scripts/obsidian-miniflare-smoke.ts` は実 Obsidian harness で plugin が作った binary file を upload し、remote client 相当で worker の manifest/chunks から同じ bytes を reassemble できること、modify で同じ fileId の manifest が更新されること、rename で同じ fileId の path が更新されること、delete で tombstone が remote へ伝わること、binary restore repair check が tombstone を復元して bytes を再 materialize すること、invalid-meta inspect/discard が隔離 detail を表示して confirmation なしでは拒否され confirmation ありで Y.Map と repair log から消えること、path-conflict retry/resolve が meta path に合わせて vault file を materialize rename し repair log を消すこと、remote-materialize-blocked resolve/retry/clear action が repair log を消して再 materialize を queue すること、remote が公開した binary を plugin が vault に materialize して hash/size が一致することを固定している。
...
- 現在の binary e2e は remote peer が HTTP で PUT して meta を公開する経路と、Obsidian plugin 自身が vault watcher から binary create/modify/rename/delete を扱う経路の両方を検証する。Obsidian plugin 側は upload/download/materialize の outbox plan、fake harness side-effect 実行、実 Obsidian harness の byte-level upload/download/materialize、modify/rename/delete、binary restore repair check、invalid-meta inspect/discard、path-conflict retry/resolve regression、remote-materialize-blocked resolve/retry/clear action を同一 manifest で固定した。残りは大きい添付向け multipart upload と、binary conflict/repair UI の追加 regression。

P1: meta materialize の残りを埋める。

- `packages/obsidian-plugin/src/main.ts` は remote meta の text entry から欠損 Markdown を作り、親フォルダがない path は作成する。既存 folder/file 衝突や invalid path は persistent repair log に出し、invalid meta は settings panel で隔離 detail を inspect してから確認付きで破棄できる。remote materialize blocked は settings panel から conflict path へ自動移動して retry できる。
- active file が remote で rename された場合は `fileManager.renameFile` 後に active binding を追従し、delete された場合は editor buffer を維持して tombstone notice を出す。rename materialize 失敗は persistent repair log に出し、settings panel から retry または conflict path resolve を実行できる。
- `packages/obsidian-plugin/src/sync/meta/reconcile.ts` の delete-vs-edit repair は、production で manifest/chunk HEAD と size を確認した restorable binary set を渡す。size 不明、HEAD 失敗、manifest/chunk 欠損、setup/token 欠損は degraded detail として settings panel に出し、再試行でまだ復元不能な場合は repair entry を保持する。
- `keep-deleted`、invalid meta entry、remote materialize blocked は persistent repair log と settings panel に出る。invalid meta は隔離 detail を inspect して確認付きで Y.Map から破棄でき、remote materialize blocked は conflict path resolve / retry / clear できる。path-conflict は conflict path resolve / path materialize retry / clear、keep-deleted は binary restore check retry/clear を settings panel から実行できる。binary restore check、degraded binary restore check、invalid-meta inspect/discard、path-conflict retry/resolve、remote-materialize-blocked resolve/retry/clear action は実 Obsidian harness で固定し、invalid-meta inspect と remote-materialize-blocked/path-conflict action は fake port harness でも固定した。
...
- `packages/worker/src/db/retention.ts` の snapshot retention plan は production checkpoint 後に実行され、R2 snapshot deletion の成功/失敗を `snapshot_retention_events` に記録する。失敗した削除は次回 cleanup で再試行される。`GET /admin/retention` は直近の retention event を返す。残りは retention policy の運用設定、event pagination、alerting。
- quarantine admin は Worker HTTP と plugin settings panel の両方にあり、server-side quarantined update の list / inspect / discard dry-run / force-apply dry-run / confirmation execute を実行できる。plugin 側 HTTP helper は schema guard、target echo guard、confirmation token 送信を unit test で固定した。残りは force-apply 後の user-facing audit summary と大量 quarantine 向け pagination。
...
現在の `packages/core` 実装は、この wire 境界の最小ランタイム検証を持つ。
...
`packages/core/src/health.ts` は `GET /health` response guard を持つ。`isHealthResponse` は supported `protocolVersion`、non-negative `checkedAt`、重複のない bounded check 配列、`status: "ok" | "degraded"` を検証する。top-level `status` が `ok` のときに degraded check が混じる response は拒否し、Worker/DO/SQLite/R2/migrations 以外の check 名は公開 protocol として受け付けない。
...
`packages/core/src/utils/errors.ts` はこの `ApiError` 形式の guard と `isRetryableApiError` を持つ。`retryAfterMs` は数値だけを許し、unknown な `code` は retryable として扱わない。Worker はこの型を唯一の公開エラー形式に寄せ、Obsidian 側は `retryable` と `retryAfterMs` だけで backoff 判定できるようにする。
...
3. `packages/core/src/sync/snapshot.ts` の `decodeFullSnapshotBytesFromResponse(input)` で `updateBytesBase64` と `stateVector` を bytes に戻し、size limit と SHA-256 が `updateSha256` / `stateVectorSha256` と一致することを確認する。`invalid-base64` / `hash-mismatch` / `state-vector-hash-mismatch` は retry で直らない可能性が高いので repair log に残し、別 generation へ fallback する。
...
現在の `packages/worker/src/db/schema.ts` 実装は、初期 SQLite schema を `INITIAL_SCHEMA_OBJECTS` と `SCHEMA_MIGRATIONS` として構造化している。初期 migration は `schema_migrations`、`setup_tokens`、`devices`、`device_refresh_tokens`、`docs`、`op_log`、`message_dedup`、`checkpoint_runs`、`connected_devices`、`quarantined_updates` と、setup token lookup / refresh token lookup / op log scan / checkpoint recovery 用 index を作る。`docs` は `latest_seq` を持ち、runtime の append path はこの clock を読み書きして次 seq を決める。`op_log` は `(doc_id, seq)` primary key と `(doc_id, message_id)` unique を持ち、再送の永続重複と seq 順序を DB 制約でも固定する。

現在の `packages/worker/src/db/migrations.ts` 実装は、この判定を `decideSchemaMigration(input)` という純粋 decision にしている。戻り値は `ready`、`apply-migrations`、`degraded` のいずれかで、`migration-failed`、`invalid-migration-plan`、`unknown-applied-migration`、`non-contiguous-applied-migrations` を区別する。`schemaAcceptsSync(decision)` は `ready` のときだけ true になる。

現在の `packages/worker/src/http/health.ts` 実装は、DO/SQLite/R2 の availability と migration decision から `decideWorkerHealth(input)` で public health response を組み立てる。`healthAcceptsSync(health)` は DO/SQLite/migrations が ok のときだけ true になり、R2 degraded だけでは durable op append を止めない。`healthAcceptsCheckpoint(health)` は R2 も ok のときだけ true になる。さらに `decideDurableObjectSyncAdmission(input)` は各 DO ローカルの SQLite availability と migration decision から WS/op append の受理可否を決める。global `/health` は概況、DO startup gate が vault 単位の sync 権威、という責務分離にする。
...
現在の `packages/core/src/sync/blob.ts` 実装は、manifest JSON の最小 schema guard を持つ。
