...
- `packages/worker/src/db/retention.ts` は snapshot retention plan を持つが、定期実行・監査ログ・削除失敗 retry が production path に薄い。checkpoint 後の retention cleanup と admin visibility を足す。
